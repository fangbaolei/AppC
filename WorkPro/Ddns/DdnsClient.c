...
// DDNS的socket通信
int DdnsSocketCom( struct hostent *phstt,char DdnsCmdBuf[1024])
{
...
int DdnsClient(void)
{
	...
	const char *DomainName = "ddns.oray.com";
	char ExIp[16] = {0};
	...
	printf("ipaddr:%s\n" ,ExIp);
	snprintf(DdnsCmdBuf,sizeof(DdnsCmdBuf)-1,"GET /ph/update?myip=%s HTTP/1.0\r\n""Host: ddns.oray.com\r\n""Authorization: Basic %s\r\n""User-Agent: Oray""\r\n\r\n",ExIp,Dst);
	printf("DDNS_CMD:\n%s",DdnsCmdBuf);
	 // 通过域名获取DDNS服务器的IP
	...
	DdnsSocketCom(phstt,DdnsCmdBuf);
